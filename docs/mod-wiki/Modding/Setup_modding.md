<!-- source: https://eu5.paradoxwikis.com/Setup_modding revid: 34660 fetched: 2026-09-09 -->
# Setup modding
**Setup** modding involves creating a "savefile" for the game to open in order to play the game. This is achieved by using a set of managers and functions inside of them which are similar, but not exactly the same, as the ones you might find in [Save-game editing](/Save-game_editing "Save-game editing").
## File structure
...
Examples include:
/Europa Universalis V/game/in\_game/setup/countries/anatolia.txt
/Europa Universalis V/game/main\_menu/setup/templates/amerindian\_monarchy.txt
/Europa Universalis V/game/main\_menu/setup/start/02\_core.txt
## Technical details
...
Here, the religion can be given [timed modifiers](/Setup_modding#timed_modifiers "Setup modding").
Example:
```
religion_manager = {
	catholic = {
...
```
### Character DB
`character_db` is used to create characters for the game start as well as their ancestors. As some of the characters will require a dynasty, it is necessary for that dynasty to be created earlier in the dynasty\_manager.
...
Some things regarding locations are set up outside setup, for instance raw materials are declared in [location\_templates](/index.php?title=Location_templates&action=edit&redlink=1 "Location templates (page does not exist)").
#### Pop definitions
...
```
#### Building templates
While individual buildings can be assigned with plenty of customization using [building manager](/Setup_modding#building_manager "Setup modding"), `locations` also allows to add buildings to locations with reusable templates using `town_setup`
Example:
```
locations = {
	stockholm = {
...
```
##### Town setups
Town setups are declared in /Europa Universalis V/game/in\_game/common/town\_setups.
Example:
```
...
```
#### Locations modifiers
Locations can be given modifiers with the [timed modifiers](/Setup_modding#timed_modifiers "Setup modding").
Example:
```
locations = {
	stockholm = {
...
| **own\_control\_core** | core | Yes | Yes |
| **own\_control\_integrated** | integrated | Yes | Yes |
| **own\_control\_conquered** | conquered | Yes | Yes |
| **own\_control\_colony** | colony | Yes | Yes |
| **own\_core** | integrated | No | Yes |
| **own\_conquered** | conquered | No | Yes |
| **own\_integrated** | integrated | No | Yes |
| **own\_colony** | colony | No | Yes |
| **control\_core** | core | Yes | No |
| **control** | none | Yes | No |
| **our\_cores\_conquered\_by\_others** | core | No | No |
...
GUC = {
	...
	add_pops_from_locations = {
		ma_tehua_llan mazatlpilli guanchenis mazatepec miopacoas galeana
	}
```
...
```
## Timed modifiers
Several types in the setup managers allow for the addition of `timed_modifiers` which allow to add starting [static\_modifiers](/index.php?title=Static_modifier&action=edit&redlink=1 "Static modifier (page does not exist)") to various objects.
...
```
## Countries folder
Outside the [countries](/Setup_modding#country_setup "Setup modding"), there is also a specialized folder for **country\_definitions** in `setup/countries`. Files here are used to give countries colors, unit colors, default cultures and religions and regnal names.
...
| Scripted content | [Actions](/Action_modding "Action modding") • [Disasters](/Disaster_modding "Disaster modding") • [Events](/Event_modding "Event modding") • [Missions](/Mission_modding "Mission modding") • [Modifiers](/Modifier_modding "Modifier modding") • [Scripted gui](/Scripted_gui "Scripted gui") • Setup • [Situations](/Situation_modding "Situation modding") • [Customizable localization](/Localization#Customizable_Localization "Localization") |
